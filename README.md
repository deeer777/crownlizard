...
Build 99 hardens the Crown Network. Runs receive rotating, server-hashed checkpoint tickets every 20 seconds, expire after eight hours and permit only one active authenticated run per account. Final telemetry must match the checkpoint chain before shards, progression or highscores are accepted; suspicious submissions are quarantined. Global Warden uses equivalent phase tickets, score creation and run consumption share one database transaction, market listing limits are serialized, legacy wallet import is removed and refresh credentials live only in a Secure HttpOnly cookie.

For production, apply `supabase/security-hardening-build99.sql` before publishing the Build 99 Worker. New environments should use the generated `supabase/canonical-schema.sql`; regenerate it after schema changes with `npm run schema`. `npm run build` creates an allowlisted `dist/` containing only public game files, keeping SQL, tests and internal documentation outside the CDN. `wrangler.jsonc` pins that output boundary and the Cloudflare compatibility date using Pages-supported configuration only.
...
### Crown Duel — PvP Pass 1–3

`crownlizard-pvp` is a separate Cloudflare Worker with one SQLite-backed Durable Object per duel room; the Pages API reaches it through the `DUEL_ROOMS` binding. Supabase stores the private discovery index and exposes only sanitized Pilot Card data through the Pages API. Apply `supabase/pvp-lobby-build100.sql` before enabling the PvP UI.

Build 100 adds the mobile-first arcade lobby. Signed-in pilots can create a challenge, browse open signals or join with an eight-character invite. Host and challenger see both callsigns and equipped ships side by side, share the invite link, transmit independent Ready state and reconnect to the same lobby after closing or reloading. Presence and Ready are server-owned, internal account IDs never enter the browser contract, and a challenger can release the seat before launch without cancelling the host's room.

Build 101 adds Pass 3: the Durable Object offers three normalized temporary blueprints, locks both loadouts, creates a cryptographic match seed and schedules one shared start and end time. Both clients run the same deterministic 90-second wave plan and exchange bounded, monotonic provisional scores through the authenticated API. The live HUD stays compact; pausing and background throttling cannot extend the server-timed race. Pass 4 still owns authoritative replay verification, winner settlement, rematch and history, so no rating, shards or rewards are issued yet.

`npm run pvp:check` regenerates binding/runtime types, type-checks the Worker, runs the real Workers-runtime Durable Object suite and performs a deployment dry-run. The production command publishes the PvP Worker before Pages so the external Durable Object binding always exists when the site is uploaded. Authoritative winner settlement, shards, rating and rewards remain outside this pass.
