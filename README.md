...
For production, apply `supabase/security-hardening-build99.sql` before publishing the Build 99 Worker. New environments should use the generated `supabase/canonical-schema.sql`; regenerate it after schema changes with `npm run schema`. `npm run build` creates an allowlisted `dist/` containing only public game files, keeping SQL, tests and internal documentation outside the CDN. `wrangler.jsonc` pins that output boundary, the Cloudflare compatibility date and invocation observability.
