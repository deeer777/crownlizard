...
assert.match(index, /src="\.\/src\/bootstrap\.js\?v=20260909-115-lean-pixels"/, 'the CSP-compliant bootstrap ships behind a fresh browser cache key');
assert.doesNotMatch(index, /<script\s+type="module">/, 'production cannot rely on executable inline modules blocked by its CSP');
assert.match(bootstrap, /main\.js\?v=20260909-115-lean-pixels/, 'the current frontend ships behind the bootstrap cache boundary');
...
assert.match(serverApi, /SUPPORTED_GAME_VERSIONS = new Set\(\['0\.46\.0-114', '0\.46\.1-115'\]\)/, 'only the current and previous production build can register server-owned runs');
