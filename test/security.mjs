...
const contact = readFileSync(new URL('../contact/index.html', import.meta.url), 'utf8');
const buildSite = readFileSync(new URL('../tools/build-site.mjs', import.meta.url), 'utf8');
...
assert.match(index, /src="\.\/src\/bootstrap\.js\?v=20260910-117-crown-cadence"/, 'the CSP-compliant bootstrap ships behind a fresh browser cache key');
assert.doesNotMatch(index, /<script\s+type="module">/, 'production cannot rely on executable inline modules blocked by its CSP');
assert.match(bootstrap, /main\.js\?v=20260910-117-crown-cadence/, 'the current frontend ships behind the bootstrap cache boundary');
...
assert.match(privacy, /Google AdSense[\s\S]*cookies, web beacons, IP addresses or other identifiers/, 'the privacy policy discloses Google advertising data technologies');
assert.match(privacy, /consent, not consent or manage individual options/, 'the privacy policy explains the three-choice European consent flow');
assert.match(contact, /Never send your password, verification link, recovery token or authentication code/, 'the support page warns players not to disclose account secrets');
assert.match(buildSite, /'contact'/, 'the allowlisted production build includes the contact page');
...
assert.match(serverApi, /SUPPORTED_GAME_VERSIONS = new Set\(\['0\.47\.0-116', '0\.48\.0-117'\]\)/, 'only the current and previous production build can register server-owned runs');
...
assert.match(main, /const sponsoredOffersEnabled = localEconomy && \(localPreview \|\| PLATFORM\.capabilities\.rewardedAds\)/, 'portal rewards stay absent until the explicit platform capability is enabled');
