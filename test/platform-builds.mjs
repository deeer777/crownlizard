...
assert.match(crazyPlatform, /"localProgression": true/);
assert.match(crazyPlatform, /"progressSave": true/);
assert.match(crazyPlatform, /"vault": true/);
assert.match(crazyPlatform, /"store": true/);
assert.match(crazyPlatform, /"market": false/);
...
assert.match(crownIndex, /FREE BROWSER ARCADE SHOOTER/);
assert.match(crownIndex, /class="arcade-site-links"/);
assert.doesNotMatch(crazyIndex, /FREE BROWSER ARCADE SHOOTER|class="logo-descriptor"|class="arcade-site-links"/);
assert.doesNotMatch(crazyIndex, /href="\/(?:about|updates)\//);
...
assert.match(main, /const localEconomy = localPreview \|\| \(PLATFORM\.capabilities\.localProgression && !serverEconomy\)/);
assert.match(crazyIndex, /id="menuVault"[\s\S]*data-platform-capability="vault"/);
assert.match(crazyIndex, /id="vaultStoreTab"[\s\S]*data-platform-capability="store"/);
assert.match(crazyIndex, /id="vaultMarketTab"[\s\S]*data-platform-capability="market"/);
assert.match(styles, /\[data-platform-capability\]\[hidden\]\s*\{\s*display:\s*none !important;/, 'platform-disabled controls cannot be re-shown by later UI renders');
