...
assert.match(index, /id="menuWardenState"[\s\S]*id="menuWardenCountdown"/, 'the title menu reserves explicit event state and countdown fields');
...
assert.match(main, /syncBossServerClock\(payload\.serverTime\)/, 'all visible Warden countdowns follow the server clock');
