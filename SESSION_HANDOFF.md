...
Current production version: **58** (`VER 0.15.3`)
Latest completed local version: **66** (`VER 0.16.1`)
Test URL: `http://127.0.0.1:4174/?debug=1&touch=1&reload=62-cookie-handoff`

Build 45 deployed the server-wallet cutover. Build 46 fixed successful empty Supabase REST responses. Build 47 made mobile startup retryable and storage-tolerant. Build 48 replaces stale anonymous mobile sessions whose refresh token is no longer accepted. Build 49 raised the hourly anonymous bootstrap cap from 20 to 60 for shared IPs. Build 50 made game start independent from Vault and stopped hidden title-screen canvas rendering. Build 51 relinked a clean anonymous wallet before background run registration. Build 52 extended cold-Auth timeouts and exposed connection phases. Live Samsung tracing then proved session creation returned 201 but the client never sent its separate wallet GET. Build 53 replaces that fragile boundary with one atomic `/api/player/bootstrap` response containing both session and wallet. Build 54 replaces oversized menu/game/crate sources with display-sized runtime sprites and makes the logo and ship entrances shorter and compositor-smooth. Build 55 adds permanent email/password player accounts while preserving the anonymous user's ID, wallet and inventory. Build 56 accepts token-bearing and confirmation-only Supabase redirects. Build 57 replaces that fallback with a branded token-hash callback. Live Auth logs proved the token exchange returned 200 and the account became permanent, but the password UI still waited behind the separate Vault request. Build 58 opens the account overlay immediately, shows verification progress and presents Create Password as soon as the token exchange succeeds while wallet loading continues independently. Build 59 adds a mobile-friendly Forgot Password action and a privacy-safe server recovery endpoint. Build 60 adds a same-origin server callback. Live audit data then proved the recovery token was successfully exchanged while the email browser still lost the redirected URL fragment. Build 61 makes callback GET requests non-consuming and presents a branded Continue screen. Build 62 removed URL credentials but the mobile browser still dropped the redirect cookie before the game could exchange it. Build 63 removes that cross-page handoff entirely: after LINK READY, the server renders Create Password directly and saves it through a narrowly scoped one-time cookie on the same page. Supabase audit data then proved the password and later email/password logins succeeded, while the client rejected the returned session. Build 64 creates a fresh session immediately after password save, replaces the stale guest session on the completion page, and normalizes both explicit and Supabase-style session shapes. Build 65 also moves ordinary manual sign-in to the same nonce-protected server completion page, bypassing the affected client's JSON session boundary entirely. Build 66 fixes the remaining silent downgrade: missing `expires_at` no longer makes a newly saved permanent session look expired, older stored sessions are repaired, and failed permanent refreshes require a new sign-in instead of creating a guest. Resend custom SMTP is active with the verified Crown Lizard sending domain. Real ad verification remains before monetization or a player market. See `SECURITY.md`.
